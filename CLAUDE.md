...
agents-radar is a daily digest generator for the AI open-source ecosystem. A GitHub Actions cron job runs at 00:00 UTC (08:00 CST) and produces five English-language reports, published as GitHub Issues and committed Markdown files.
...
- All LLM prompts are in `src/prompts.ts`. Each report type has its own builder function. Prompts are in English and the model produces English output.
...
- `sampleNote(total, sampled)` in `src/prompts.ts` formats the "(Total: N items; showing top M...)" note. Reuse it — do not inline the same string format.
...
- Reports are now English-only. The `REPORT_LANGS` env var and `lang` parameters have been removed.
